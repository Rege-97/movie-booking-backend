...
public class AuthService {
     ...
     * - Redis에 저장된 Refresh Token과 일치하는지 확인 (토큰 탈취 방지)
     * - 새로운 Access/Refresh Token 발급 및 Redis 업데이트
     */
    @Transactional
    public LoginResponse reissueTokens(String refreshToken) {
        ...
        String storedRefreshToken = redisTemplate.opsForValue().get(getRefreshTokenKey(member.getId()));

        if (!refreshToken.equals(storedRefreshToken)) {
            if (storedRefreshToken != null) {
                redisTemplate.delete(getRefreshTokenKey(member.getId()));
            }
            throw new InvalidCredentialsException("Refresh Token 정보가 일치하지 않거나 만료되었습니다.");
        }

        return generateAndStoreTokens(member);
    }

    private LoginResponse generateAndStoreTokens(Member member) {
        // 토큰 발급
        String accessToken = jwtTokenProvider.generateAccessToken(member);
        String refreshToken = jwtTokenProvider.generateRefreshToken(member);

        // Redis에 Refresh 토큰 저장 및 만료 시간 설정
